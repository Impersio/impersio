...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn("h-7 w-7 flex items-center justify-center rounded-md hover:bg-sidebar-accent hover:text-sidebar-accent-foreground text-sidebar-foreground/70", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
>(({ className, ...props }, ref) => {
  return (
    <input
      ...
      data-sidebar="input"
      className={cn(
        "flex h-8 w-full bg-background rounded-md px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring",
        className
      )}
      {...props}
    />
